...
class HW2_sql():
    ...
    def part_aii_1(self,connection,path):
        ...
        with open(path, 'r') as movTable:
            reader = csv.reader(movTable)
            for item in reader:
                cur.execute("INSERT INTO movies VALUES (?,?,?);", item)
    ...
    def part_aii_2(self,connection, path):
        ############### CREATE IMPORT CODE BELOW ############################
        cur = connection.cursor()
        with open(path, 'r') as movCTable:
            reader = csv.reader(movCTable)
            for item in reader:
                cur.execute("INSERT INTO movie_cast VALUES (?,?,?,?,?);", item)
    ...
    def part_c(self,connection):
        ############### EDIT SQL STATEMENT ##################################
        part_c_sql = 'SELECT PRINTF("%.2f", CAST(SUM(CASE WHEN score > 50 AND title LIKE "%war%" THEN 1 ELSE 0 END) AS FLOAT)/ CAST(count(*) AS FLOAT) * 100) FROM movies;'
        ######################################################################
    ...
    def part_d(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_d_sql = "SELECT cast_name, count(cast_name) from movie_cast WHERE popularity > 10 GROUP BY cast_name ORDER BY count(cast_name) DESC, cast_name ASC LIMIT 5;"
        ######################################################################
    ...
    def part_e(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_e_sql = "SELECT title, PRINTF('%.2f', score), COUNT(cast_id) FROM movies, movie_cast WHERE movies.id = movie_cast.movie_id GROUP BY title ORDER BY score DESC, count(cast_id) ASC, title ASC LIMIT 5;"
        ######################################################################
    ...
    def part_f(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_f_sql = "SELECT cast_id, cast_name, PRINTF('%.2f',AVG(score)) FROM movie_cast, movies WHERE movie_cast.movie_id = movies.id AND score >= 25 GROUP BY cast_id HAVING count(*) > 2 ORDER BY AVG(score) DESC, cast_name ASC LIMIT 10;"
        ######################################################################
    ...
    def part_g(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_g_sql = """CREATE VIEW good_collaboration AS SELECT [1st].cast_id AS cast_member_id1, [2nd].cast_id AS cast_member_id2, count([1st].movie_id) AS movie_count, AVG(score) AS average_movie_score
                        FROM ((SELECT * FROM movie_cast WHERE movie_id != '')[1st]
                        INNER JOIN (SELECT * FROM movie_cast WHERE movie_id !='')[2nd] ON [1st].movie_id = [2nd].movie_id)
                        INNER JOIN (SELECT * FROM movies WHERE id !='') ON [1st].movie_id = id
                        WHERE [1st].cast_id < [2nd].cast_id GROUP BY cast_member_id1, cast_member_id2 HAVING movie_count > 2 AND AVG(score) > 39;"""
        ######################################################################
    ...
    def part_gi(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_g_i_sql = """SELECT cast_id, cast_name, PRINTF( '%.2f', AVG(score1 || score2)) AS collaboration_score FROM (((SELECT * FROM movie_cast)
                        INNER JOIN (SELECT cast_member_id1, average_movie_score AS score1 FROM good_collaboration) ON cast_id = cast_member_id1)
                        INNER JOIN (SELECT cast_member_id2, average_movie_score AS score2 FROM good_collaboration) ON cast_id = cast_member_id2)
    ...
    def part_h(self,connection,path):
        ...
        ############### CREATE IMPORT CODE BELOW ############################
        cur = connection.cursor()
        with open(path, 'r') as movOTable:
            reader = csv.reader(movOTable)
            for item in reader:
                cur.execute("INSERT INTO movie_overview VALUES (?,?);", item)
